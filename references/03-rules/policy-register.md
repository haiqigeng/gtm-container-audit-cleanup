...
| POL-305 | D2/D3 proof must start with literal object behavior and actual consumers before category or judgment. Generic evidence signals or fake-precision categories such as `custom code inspected`, `returns computed value`, `payload transformer`, `browser side effect`, `see config`, or `see export` do not count as semantic proof. |
